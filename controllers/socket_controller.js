...
const handlePlayerJoined = function (username, callback) {
	debug(
		`User ${username} with socket id ${this.id} has connected to the first room: room ${rooms}`
	);
	...
	this.join(rooms);
	debug(
		`User ${username} with socket id ${this.id} has connected to ONE PERSON JOINED ROOM ${rooms}`
	);
	...
	if (Object.keys(players).length === 2) {
		...
		debug(
			`User ${username} with socket id ${this.id} has connected 2 PEOPLE HAVE CONNECTED to room ${rooms}`
		);

		debug(Object.keys(players));
		...
		//io.emit('start game');
		this.emit('newGame', player1, player2);
		//rooms = KULRUM;
		debug(
			`Players ${gamesArray.thisGame} .User ${username} with socket id ${this.id} is in room ${rooms}.`
		); //please write out the players here
